	...
	while ( IsValid(circle) )
	{
		...
		foreach ( dummy in GetNPCArray() )
		{
			...
			if ( playerDist < currentRadius )
			{
				...
				dummy.TakeDamage( damagetodeal, owner, null, { scriptType = DF_BYPASS_SHIELD | DF_DOOMED_HEALTH_LOSS, damageSourceId = eDamageSourceId.deathField } )
			}
		}
		...
		foreach ( player in GetPlayerArray_Alive() )
		{
			...
			if ( playerDist < currentRadius )
			{
				...
				player.TakeDamage( damagetodeal, owner, null, { scriptType = DF_BYPASS_SHIELD | DF_DOOMED_HEALTH_LOSS, damageSourceId = eDamageSourceId.deathField } )
			}
		}
